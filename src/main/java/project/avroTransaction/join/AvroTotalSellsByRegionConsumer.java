...
public class AvroTotalSellsByRegionConsumer {

    public static void main(String [] args) {
        Schema schemaP = SchemaAvro.getPharmSchema();
        Injection<GenericRecord, byte[]> recordInjection = GenericAvroCodecs.toBinary(schemaP);

        Schema schema = SchemaAvro.getSchema();
        Injection<GenericRecord, byte[]> recordInjection2 = GenericAvroCodecs.toBinary(schema);

        Schema schemaMerge = SchemaAvro.getMergedSchema();
        GenericRecord recordM = new GenericData.Record(schema);
        Injection<GenericRecord, byte[]> recordInjectionMerge = GenericAvroCodecs.toBinary(schemaMerge);

        TopicCreator.createTopic();
        //AtomicInteger i= new AtomicInteger(0);
        Properties props=new Properties();
        props.put(StreamsConfig.APPLICATION_ID_CONFIG, "firstTestp.Consumer");
        ...
        KStream<String, byte[]> pharm=builder.stream("testpharm");

        KStream<String, byte[]> merge = pharm.leftJoin(sourceProcessor,
        ...
        /*pharm.foreach((x, y) -> {
            try {
                GenericRecord record = recordInjection.invert(y).get();
                    Pharm p = new Pharm(
                            Integer.parseInt(String.valueOf(record.get("id"))),
                            String.valueOf(record.get("nom")),
                            String.valueOf(record.get("address")),
                            String.valueOf(record.get("depart")),
                            String.valueOf(record.get("region")));
